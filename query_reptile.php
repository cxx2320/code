...
$data = QueryList::html($html)->rules($rules)->query()->getData(function ($item) {
    $item['img'] = QueryList::html($item['img'], array(
        'img' => array('.default-img', 'data-src')
    ))->data;

    $item['yanyuan'] = QueryList::html($item['yanyuan'], array(
        'cover' => array('li>a .default-img', 'data-src'),
        'name' => array('li>.info .name', 'text'),
        'role' => array('li>.info .role', 'text'),
    ))->data;

    $item['daoyan'] = QueryList::html($item['daoyan'], array(
        'cover' => array('li a .default-img', 'data-src'),
        'name' => array('li .info a', 'text'),
    ))->data;

    $item['comment'] = QueryList::html($item['comment'], array(
        'img' => array('ul li .portrait-container img', 'src'),
        'name' => array('ul li .main .user .name', 'text'),
        'time' => array('ul li .main .time', 'title'),
        'level' => array('ul li .main .time ul', 'data-score', '', function ($level) {
            return floor($level / 2);
        }),
        'content' => array('ul li .main .comment-content', 'text')
    ))->data;
    return $item;
});
var_dump($data->all());
